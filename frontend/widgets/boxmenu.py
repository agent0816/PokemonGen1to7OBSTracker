...
class BoxMenu(Screen):
    def __init__(self, bizhawk, citra, munchlax, obs_websocket, pl, **kwargs):
        ...
        # Slot-Grid (5 Zeilen × 6 Spalten). Detail-Ansicht liegt als Top-Level-
        # Screen "PokemonDetail" im ScreenManager der App — dorthin navigiert
        # _show_slot_detail und der Zurück-Callback bringt uns wieder her.
    ...
    def _show_slot_detail(self, slot_index):
        ...
        detail = self.manager.get_screen("PokemonDetail")
        manager = self.manager
        prev_screen = manager.current
        detail.show(
            pokemon, edition, rando_data,
            back_callback=lambda: setattr(manager, 'current', prev_screen),
            munchlax=self.munchlax,
        )
        manager.current = "PokemonDetail"
